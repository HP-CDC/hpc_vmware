...
sub create_vm {
   ...
   my $controller = VirtualLsiLogicController->new(key => 0,
                           device => [0],
                           busNumber => 0,
...
# Find dvportgroup
   my $dvportgroup_view = Vim::find_entity_view(
	...
	# properties => ['name', 'key', 'config.distributedVirtualSwitch'],
	 ...
	 unless ($dvportgroup_view) {
			Util::trace(0,"\nUnable to locate $network_name in Datacenter: '" . $datacenter->name . "'!\n");
	        Util::disconnect();
			exit(0);
		}

	for my $network_name ($dvportgroup_view) {
		if (grep $_ eq $network_name, $dvportgroup_view) {	

			# Retrieve dvportgroup and DVS reference object
	   		...
	   		my $virtual_device_backing_info = VirtualEthernetCardDistributedVirtualPortBackingInfo->new(
           		...
           		my $nic = VirtualVmxnet3->new(backing => $virtual_device_backing_info,    
	                		              	key => 0,
                                        		unitNumber => $unit_num,
